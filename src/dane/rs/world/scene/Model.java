package dane.rs.world.scene;

import dane.rs.*;
import dane.rs.data.*;
import dane.rs.io.*;
import dane.rs.media.*;
import dane.rs.util.*;
import dane.rs.world.scene.model.*;
...
public class Model extends CacheableNode {
	...
	public static void load(Archive a) {
		try {
			...
			for (int n = 0; n < count; n++) {
				int index = obhead.getUShort();
				Metadata m = meta[index] = new Metadata();

				m.vertexCount = obhead.getUShort();
				m.triangleCount = obhead.getUShort();
				m.texturedCount = obhead.getUByte();
	...
	public void drawTriangle(int index) {
		if (projectTriangle[index]) {
			...
			if (type == 0) {
				Graphics3D.fillShadedTriangle(vertexScreenX[a], vertexScreenY[a], colorA[index], vertexScreenX[b], vertexScreenY[b], colorB[index], vertexScreenX[c], vertexScreenY[c], colorC[index]);
			} else if (type == 1) {
	...
	private void drawProjectedTriangle(int index) {
		...
		if (((xA - xB) * (yC - yB) - (yA - yB) * (xC - xB)) > 0) {
			...
			if (n == 3) {
				...
				if (type == 0) {
					Graphics3D.fillShadedTriangle(xA, yA, tmpColor[0], xB, yB, tmpColor[1], xC, yC, tmpColor[2]);
				} else if (type == 1) {
			...
			if (n == 4) {
				...
				if (type == 0) {
					Graphics3D.fillShadedTriangle(xA, yA, tmpColor[0], xB, yB, tmpColor[1], xC, yC, tmpColor[2]);
					Graphics3D.fillShadedTriangle(xA, yA, tmpColor[0], xC, yC, tmpColor[2], tmpX[3], tmpY[3], tmpColor[3]);
				} else if (type == 1) {
